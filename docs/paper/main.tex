...
\begin{tikzpicture}[node distance=0.5cm]
  ...
  \node[data, right=0.8cm of ui] (api) {FastAPI :8000 (CORS)};
  % Row 2: orchestration (single wide node)
  \node[proc, below=0.6cm of $(ui)!0.5!(api)$, minimum width=6.4cm] (orch)
       {LangGraph StateGraph (typed AgentState)};
  % Row 3: stores + fusion
  \node[store, below left=0.6cm and -1.0cm of orch.south] (qdr) {Qdrant vectors};
  \node[store, below=0.6cm of orch] (neo) {Neo4j graph};
  \node[store, below right=0.6cm and -1.0cm of orch.south] (rrf) {RRF fusion};
  % Row 4: reasoning
  \node[data, below=0.65cm of $(qdr)!0.5!(neo)$] (llm) {OpenRouter LLM (tools)};
  \node[proc, right=0.7cm of llm] (val) {Validator--Corrector};
  % Row 5: persistence
  \node[store, below=0.65cm of $(llm)!0.5!(val)$] (sql) {SQLite audit ledger};
  % Edges
  \draw[arr] (ui.south) -- (orch);
  \draw[arr] (api.south) -- (orch);
  \draw[arr] (orch.south) -- (qdr.north);
  \draw[arr] (orch.south) -- (neo.north);
  \draw[arr] (orch.south) -- (rrf.north);
  \draw[arr] (rrf.south) -- (llm);
  \draw[arr] (llm) -- (val);
  \draw[arr] (val) -- (llm);
  \draw[arr] (llm.south) -- (sql);
  \draw[arr] (val.south) -- (sql);
\end{tikzpicture}%
}
...
\begin{tikzpicture}[node distance=0.45cm]
  ...
  \draw[arr] (hyb) -- (reason);
  \draw[arr] (reason) -- (ans);
  \draw[arr] (reason.south) -- (corr.west |- reason.south);
  \draw[arr] (corr) -- (pass);
  \draw[arr] (pass.west) to[out=180,in=180] node[lab, left]{no} (hyb.west);
  \draw[arr] (corr.east) to[out=0,in=0] node[lab, right]{yes} (ans.south east);
